...
class ShotPercentChart extends React.Component {
  ...
  render() {
    ...
    const netsData = [
      ...
      { x: 2, y: nets.twoP, label: `${nets.twoP}` },
      { x: 3, y: nets.thrP, label: `${nets.thrP}` },
      { x: 4, y: nets.ftP, label: `${nets.ftP}` }
    ]

    const leagueData = [
      { x: 1, y: league.fgp, label: `${league.fgp}` },
      { x: 2, y: league.twoP, label: `${league.twoP}` },
      { x: 3, y: league.thrP, label: `${league.thrP}` },
      { x: 4, y: league.ftP, label: `${league.ftP}` }
    ]

    const oppData = [
      { x: 1, y: opponent.fgp, label: `${opponent.fgp}` },
      { x: 2, y: opponent.twoP, label: `${opponent.twoP}` },
      { x: 3, y: opponent.thrP, label: `${opponent.thrP}` },
      { x: 4, y: opponent.ftP, label: `${opponent.ftP}` }
    ]
      ...
      <div className='barChart'>
        <Paper>
          <VictoryChart containerComponent={<VictoryZoomContainer/>}>
            <VictoryLegend x={60} y={0} title='Shooting %s' centerTitle orientation='horizontal' gutter={20} data={legendData} style={{title: { fontSize: 20}}}/>
            <VictoryGroup domain={{x: [0, 5], y:[0.2, 1]}} offset={20} colorScale={"qualitative"} categories={{x: ['FG%', '2pt%', '3pt%', 'FT%']}}>
